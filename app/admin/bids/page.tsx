          ...
          <div className="flex flex-col gap-3 md:hidden">
            {filtered.map((bid) => {
              ...
              return (
                <div
                  key={bid._id}
                  className={`bg-white rounded-xl border shadow-sm p-4 ${isTopBid ? "border-green-200" : "border-gray-100"}`}
                >
                  {/* Bidder row */}
                  <div className="flex items-start justify-between gap-2 mb-3">
                    <div>
                      <p className="font-semibold text-gray-900 text-sm">{bid.bidder?.name ?? "—"}</p>
                      <p className="text-xs text-gray-400">{bid.bidder?.email ?? "—"}</p>
                    </div>
                    <div className="text-right shrink-0">
                      <p className={`font-bold text-base ${isTopBid ? "text-green-600" : "text-gray-900"}`}>
                        {formatCurrency(bid.amount)}
                      </p>
                      {isTopBid && (
                        <span className="text-[10px] bg-green-100 text-green-700 px-1.5 py-0.5 rounded font-medium">Highest</span>
                      )}
                      {bid.isAutoBid && (
                        <span className="text-[10px] text-blue-500 ml-1">(auto)</span>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="bg-gray-50 rounded-lg px-3 py-2 mb-3">
                    <p className="text-xs text-gray-400 mb-0.5">Auction</p>
                    <p className="text-sm font-medium text-gray-800 line-clamp-1">{bid.auction?.title ?? "—"}</p>
                  ...
                  <div className="flex items-center justify-between gap-2 flex-wrap">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <span className="text-xs text-gray-400">{fmt(bid.createdAt)}</span>
                    </div>
                    ...
                    {canSetWinner && bid.auction?.status !== "ended" && (
                      <button
                        ...
                        className="flex items-center gap-1 px-3 py-1.5 bg-[#2874F0] text-white text-xs font-semibold rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-60"
                      >
